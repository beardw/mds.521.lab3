...
**eval = FALSE** is preventing the lines from being executed at all at the time of knitting.
...
```
**results = 'hide'** makes it so that the code hides the results from the execution of the chunk. 

**message = FALSE** hides the messages produced, for instance, from prints and warnings.
...
Link: https://github.ubc.ca/mds-2025-26/DSCI_521_lab3_group20/blob/master/question_9.md
In the preview it is rendered as an R markdown environment. So it carries the order as the 
visual .Rmd documents are displayed. The code part looks like the source .Rmd file but
with more order, not as detailed either.
